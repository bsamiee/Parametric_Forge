...
- `config.forge.ssh` shape: `{ hosts.<name>.{ name, user, hostName, aliases, tunnelHost, forwards, mounts }, identityAgent, mountRoot; }` — mount rows carry `{ name, path, readOnly, cache, mountpoint }`; `identityAgent` is the 1Password socket every remote consumer pins; cache posture sinks only into the rclone `forge-vps-mount` agents (Yazi's SFTP schema has no cache field). Mount agents prove liveness continuously and emit `mounted|down|reaped` transitions the standing-alert lane and receipt registry judge; the workspace picker derives one remote row per (host, mount).
...
- [01]-[ATTENTION]: `.claude/hooks/agent-attention.sh` and the WezTerm bell arm (`deck.lua M.attention`, fired by `events.lua` on background rings) append JSONL rows (`ts, source, event, session_id, cwd, term, wezterm_pane, zellij_session, zellij_pane, tty`; `source` discriminates `hook`/`bell`) that `forge-agents collect` folds and `forge-agents focus`/`answer` route; `shell-tools/attention.nix` owns the vocabulary — urgency ladder, kv parser, event spine, and the standing-alert predicate rows (drift, tunnel) the collector judges against receipt-registry paths. Renderers on the one stream: `pipe_agents`/`pipe_quota` cells and the `{notifications}` toast by exact pipe name (`apps/zellij/config.nix`), terminal-notifier banners, the alerter answer channel (`forge-agents answer` writes the typed reply into the waiting pane by pane id), and the ntfy cross-device publish (policy rows in `notifyPolicy.remote`; target and topic ride the `NTFY_URL`/`NTFY_TOPIC`/`NTFY_TOKEN` Doppler rows, the maghz `services.ntfy-sh` loopback server standing behind the `ntfy` tunnel row as the self-hosted target); `forge-zellij peek` supplies the pane-tail context both banner and answer carry. A field rename, tty-form change, or pipe-name edit on any side severs the chain silently. Budgets ride `FORGE_ATTENTION_{FEED,MAX_ROWS,KEEP_ROWS}`.
- [02]-[RECEIPTS]: every `forge-*` kernel persists TSV receipts to `~/Library/Logs/forge-<name>.receipts.log`, override key `FORGE_<NAME>_RECEIPT_LOG` (grammar minted by the `forge-tools.nix` builder); `forge-receipts` discovers sources from `config.forge.registers.receiptSources` (rows carry `grain` — `kv` TSV or `json` JSONL) plus `config.forge.ssh.hosts` rows. The plane is queryable and live: `--sql`/`--verb` run DuckDB over the normalized event spine with the attention feed folded in as `kind=attention`, `--audit` diffs registry rows against on-disk reality and fails on unregistered emitters, and the `forge-receipts-bus` launchd agent pushes every high-urgency row into the zjstatus `{notifications}` toast as it lands. A new kernel that hand-rolls its receipt path is an `--audit` FAIL — invisible to browser, query plane, and bus alike.
...
- [04]-[XDG_PROJECTIONS]: agent-facing artifacts live at fixed projection paths — `~/.config/forge/registers/*.json` (browsers), `~/.config/forge/theme/palette.json` + `forge-dracula.tmTheme` (theme), `~/.config/forge/vscode/roster.json` (extension roster + supersession map), `~/.local/state/forge/` (collector state, attention feed), `~/.cache/forge*/` (launcher prefixes). Consumers hardcode these paths by contract; moving one is an estate-wide grep, not a local edit.
- [05]-[QA_HOOKS]: `flake-modules/qa.nix` invokes `fmt --self-test`/`--check` from `scripts/fmt.nix`; treefmt lanes and `fmt` share formatter ownership per extension — a file class both claim gets formatted twice, and a placeholder-bearing template neither may own (the `.sql.tpl` scar).
- [06]-[SESSION_FABRIC]: one workspace row (`wezterm/default.nix`) carries picker entry, zellij session identity, cwd, float policy, and warm posture; `deck.lua session_args` and `forge-workspace` both resolve frozen `~/.local/state/forge/zellij-layouts/<slug>.kdl` assets (`forge-zellij layout record`) before the default layout, `forge-workspace --json` lifecycle and `forge-zellij state` (schema v2: classification, `serialized_ts`, `last` fabric receipt) parse the same `list-sessions` EXITED text, and remote rows device-diff their mountpoint before spawning; `forge-terminal-accept.sh` R15/R16 assert both envelopes.
