...
- `config.forge.ssh` mount agents prove liveness continuously and emit `mounted|down|reaped` transitions the standing-alert lane and receipt registry judge; the workspace picker derives one remote row per (host, mount).
...
- [01]-[ATTENTION]: `.claude/hooks/agent-attention.sh` and the WezTerm bell arm (`deck.lua M.attention`, fired by `events.lua` on background rings) append JSONL rows (`ts, source, event, session_id, cwd, term, wezterm_pane, zellij_session, zellij_pane, tty`; `source` discriminates `hook`/`bell`) that `forge-agents collect` folds and `forge-agents focus`/`answer` route. A field rename, tty-form change, or pipe-name edit on any side severs the chain silently. Surfaces on the one stream:
    - `shell-tools/attention.nix` owns the vocabulary — urgency ladder, kv parser, event spine, and the standing-alert predicate rows (drift, tunnel) the collector judges against receipt-registry paths, budgets on `FORGE_ATTENTION_{FEED,MAX_ROWS,KEEP_ROWS}`.
    - `pipe_agents`/`pipe_quota` cells and the `{notifications}` toast by exact pipe name (`apps/zellij/config.nix`).
    - terminal-notifier banners, with `forge-zellij peek` supplying the pane-tail context both the banner and answer carry.
    - the alerter answer channel: `forge-agents answer` writes the typed reply into the waiting pane by pane id.
    - the ntfy cross-device publish: policy rows in `notifyPolicy.remote`, target and topic on the `NTFY_URL`/`NTFY_TOPIC`/`NTFY_TOKEN` Doppler rows, the maghz `services.ntfy-sh` loopback server behind the `ntfy` tunnel row as the self-hosted target.
...
- [06]-[SESSION_FABRIC]: one workspace row (`wezterm/default.nix`) carries picker entry, zellij session identity, cwd, float policy, and warm posture, and remote rows device-diff their mountpoint before spawning; `forge-terminal-accept.sh` R15/R16 assert both envelopes.
