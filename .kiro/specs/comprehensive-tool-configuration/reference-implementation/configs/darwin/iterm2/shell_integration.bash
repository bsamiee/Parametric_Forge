# Title         : shell_integration.bash
# Author        : Bardia Samiee
# Project       : Parametric Forge
# License       : MIT
# Path          : configs/darwin/iterm2/shell_integration.bash
# ----------------------------------------------------------------------------
# iTerm2 shell integration for bash (macOS)
...
if [[ "$TERM_PROGRAM" == "iTerm.app" ]]; then
    ...
    # Set up hooks using PROMPT_COMMAND
    if [[ -z "$ITERM2_PRECMD_SET" ]]; then
        export ITERM2_PRECMD_SET=1
        PROMPT_COMMAND="${PROMPT_COMMAND:+$PROMPT_COMMAND; }iterm2_precmd"
    fi
    
    # Set up preexec hook
    if [[ -z "$ITERM2_PREEXEC_SET" ]]; then
        export ITERM2_PREEXEC_SET=1
    ...
    it2copy() {
        if [[ $# -eq 0 ]]; then
            cat | base64 | while read line; do printf "\033]1337;Copy=:%s\007" "$line"; done
        else
            printf "%s" "$*" | base64 | while read line; do printf "\033]1337;Copy=:%s\007" "$line"; done
        fi
    }
    ...
    imgcat() {
        ...
        printf "\033]1337;File=inline=1:%s\007" "$(base64 < "$1")"
    }
    ...
    # Auto-update git status in badge (add to PROMPT_COMMAND)
    if command -v git >/dev/null 2>&1; then
        PROMPT_COMMAND="${PROMPT_COMMAND:+$PROMPT_COMMAND; }it2git"
    fi
    ...
    it2save() {
        ...
        echo "Current command: $READLINE_LINE" >> "$session_file"
    ...
    # Bash-specific key bindings for iTerm2
    bind '"\e[1;9C": forward-word'      # Option+Right
    bind '"\e[1;9D": backward-word'     # Option+Left
    bind '"\e[1;5C": forward-word'      # Ctrl+Right
    bind '"\e[1;5D": backward-word'     # Ctrl+Left
    
    echo "iTerm2 shell integration loaded (bash)"
fi