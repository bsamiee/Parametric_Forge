...
- gpt-5.5 is reachable only through the Codex CLI (`codex exec` / `codex review`); `~/.codex/config.toml` defaults it at medium reasoning — escalate a single run with `-c model_reasoning_effort="high"` (or `"xhigh"`) for the hardest research, review, and design legs. The `codex` skill owns delegation triggers, invocation mechanics, sandboxing, effort tiers, sessions, and review modes.
