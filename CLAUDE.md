...
- Storage questions ride `dust -d 1 -n 20 -r ~` and each tool's own prune verb — `uv cache prune`, `pnpm store prune`, `docker system prune`, `mise prune --configs`; store maintenance runs `sudo nix-env -p /nix/var/nix/profiles/system --delete-generations old`, `nix-collect-garbage -d`, and `nix store optimise`, and Homebrew currency runs `brew update && brew upgrade && brew upgrade --greedy-latest wezterm@nightly && brew cleanup`.
