...
- Latest nixpkgs-unstable always; a pin exists only with a named incompatibility and dies when compatibility lands. `nixpkgs-sci` is the one standing exception: it carries the slow-scientific python-overlay lane and advances only through the atlas update sequence.
...
- `forge-browse tools` is the estate's own command index — every packaged executable with its owner file and the trigger that calls for it — and the same rows land at `~/.config/forge/registers/tools.json` for a non-interactive read. Reach for it before hand-rolling a pipeline the estate already packages.
- `forge-doctor <lens>` (`path`, `launchd`, `parity`, `updates`; `--json` per lens) is the read-only machine doctor: run it before attributing a failure to the estate, because a shadowed binary, an unloaded agent, generation-versus-`$HOME` drift, and stale currency each read as a code bug from inside a session. `forge-receipts --verb`/`--sql` queries what every rail already recorded, and `--audit` fails on any emitter the registry does not claim.
- `forge-cleanup plan` proves rows before `forge-cleanup apply` trashes them; the registry carries only standing policy no first-party tool expresses. Ad-hoc storage questions ride `dust -d 1 -n 20 -r ~` and each tool's own prune verb — `uv cache prune`, `pnpm store prune`, `docker system prune`, `mise prune --configs` — never a new registry row.
