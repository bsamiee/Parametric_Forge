...
in {
  ...
  xdg.configFile =
    {
      # HARLEQUIN_CONFIG_PATH pins discovery to this file; the postgres profile rides libpq PG* environment defaults.
      "harlequin/config.toml".source = tomlFormat.generate "harlequin-config" harlequinConfig;

      # VisiData colors are curses-palette only; the default dark theme stands. VD_CONFIG/VD_DIR own discovery and state paths — the vendored appdirs
      # would otherwise resolve to ~/Library/Preferences on darwin.
