...
# hourly orphan lane). Ad-hoc storage questions ride `dust -d 1 -n 20 -r ~` and each tool's own prune verb — never registry rows.
{
  ...
  cleanupRows = pkgs.writeText "forge-cleanup-rows.json" (builtins.toJSON (projectRows {
    ...
    orphan = {
      ...
      lsp-server-orphans = ["(tsc --lsp|bash-language-server|yaml-language-server|lua-language-server|(^|/)nixd|dts-lsp|postgrestools|Microsoft[.]CodeAnalysis[.]LanguageServer|(^|/)ty server|taplo lsp stdio|biome lsp-proxy)" "" 300 ""];
