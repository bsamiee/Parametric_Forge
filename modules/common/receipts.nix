...
# ----------------------------------------------------------------------------
# Receipt grammar owner for every receipt-bearing kernel. `receiptPath` is the one derivation of an emitter's log location — the
# FORGE_<STEM>_RECEIPT_LOG override key, the $HOME-relative file the register audits, and the shell expression a kernel assigns — keyed on the
# host OS, so no reader or register respells Library/Logs. `fold` is the dual-receipt emit fold: one row spec in (a k=v TSV row or a JSON
# object), one human TSV line plus a JSONL sibling out with identical envelope keys and JSON-number numerics; `ts` is stamped at emit unless the
# row carries one, `surface` from receipt_surface. The host script sets receipt_log and receipt_surface and carries jq in runtimeInputs.
{
  receiptPath = {
    lib,
    isDarwin,
  }: name: let
    stem = lib.removePrefix "forge-" name;
    rel =
      if isDarwin
      then "Library/Logs/forge-${stem}.receipts.log"
      else ".local/state/parametric-forge/${stem}.receipts.log";
    envKey = "FORGE_${lib.toUpper (lib.replaceStrings ["-"] ["_"] stem)}_RECEIPT_LOG";
  in {
    inherit envKey rel;
    expr = "\${${envKey}:-$HOME/${rel}}";
  };

  fold = ''
    append_receipt() ( # $1: "k=v<TAB>k=v" row or one-line JSON object
      local ts tsv json receipt_dir json_log lock_file lock_fd
      ...
      # Emit-time stamp: a receipt trails its action, so a multi-row rail never reuses its start stamp; a row carrying its own ts keeps it.
      TZ=UTC0 printf -v ts '%(%Y-%m-%dT%H:%M:%SZ)T' "$EPOCHSECONDS"
      if [ "''${1:0:1}" = "{" ]; then
        json="$(printf '%s\n' "$1" | jq -ec --arg ts "$ts" --arg s "$receipt_surface" '{ts: $ts} + . + {surface: (.surface // $s)}')" || return 65
        tsv="$(printf '%s\n' "$json" | jq -er 'to_entries | map("\(.key)=\(.value | tostring)") | join("\t")')" || return 65
      else
        if [[ $'\t'"$1" == *$'\tts='* ]]; then
          printf -v tsv '%s\tsurface=%s' "$1" "$receipt_surface"
        else
          printf -v tsv 'ts=%s\t%s\tsurface=%s' "$ts" "$1" "$receipt_surface"
        fi
        json="$(printf '%s\n' "$tsv" | jq -Rc 'split("\t")
