...
const _settings = Config.all({
  passphraseRef: Config.withDefault(
    Config.nonEmptyString("FORGE_SERVICES_PASSPHRASE_REF"),
    "op://Tokens/PULUMI_FORGE_SERVICES/password",
  ),
  tokenRef: Config.withDefault(
...
const _ScopePair = Schema.parseJson(Schema.Record({ key: Schema.String, value: Schema.Unknown }));

const _scopeTable = Effect.flatMap(
  _shell("doppler", "configure", "--all", "--json"),
  Schema.decodeUnknown(_ScopeTable),
);

const _resolved = (dir: string) =>
  Effect.map(
    Effect.flatMap(
      _shell("doppler", "configure", "get", "project", "config", "--json", "--scope", dir),
      Schema.decodeUnknown(_ScopePair),
    ),
    (raw) => ({
      project: typeof raw["enclave.project"] === "string" ? raw["enclave.project"] : "",
      config: typeof raw["enclave.config"] === "string" ? raw["enclave.config"] : "",
    }),
  );

const _strayScopes = Effect.map(_scopeTable, (table) => {
  const declared = new Set<string>(scopes.map((row) => row.dir));
  return Object.entries(table)
    .filter(
      ([dir, entry]) =>
        dir.startsWith(`${scopeRoot}/`) && !declared.has(dir) && ("enclave.project" in entry || "enclave.config" in entry),
    )
    .map(([dir]) => dir);
});
...
const _strayYaml = Effect.gen(function* () {
  const fs = yield* FileSystem.FileSystem;
  const entries = yield* fs.readDirectory(scopeRoot);
  const candidates = [
    path.join(scopeRoot, "doppler.yaml"),
    ...entries.map((entry) => path.join(scopeRoot, entry, "doppler.yaml")),
  ];
  // Plain files under scopeRoot yield ENOTDIR on the probe; absence either way.
  const present = yield* Effect.forEach(
    candidates,
    (candidate) => Effect.orElseSucceed(fs.exists(candidate), () => false),
    { concurrency: 8 },
  );
...
const _applyScopes = Effect.gen(function* () {
  const fs = yield* FileSystem.FileSystem;
  const rows = yield* Effect.filter(scopes, (row) => fs.exists(row.dir), { concurrency: 4 });
  ...
  yield* Effect.forEach(
    strays,
    (dir) => _shell("doppler", "configure", "unset", "project", "config", "--scope", dir),
    { concurrency: 1 },
  );
});

const _printed = Effect.flatMap(_doctor, (report) =>
  Effect.map(Console.log(JSON.stringify(report, null, 2)), () => report),
);
...
const _targeted = (flags: Flags): { readonly target?: string[] } =>
  flags.targets.length === 0 ? {} : { target: flags.targets.map(_tokenUrn) };
...
const _verbs = {
  ...
  outputs: (flags: Flags, positional: ReadonlyArray<string>) =>
    Effect.flatMap(_stackAct("outputs", flags, (stack) => stack.outputs()), (outputs) => {
...
const _program = Effect.gen(function* () {
  ...
  const stray = argv.find(
    (arg) => arg.startsWith("--") && !FLAG_VOCABULARY.has(arg) && !arg.startsWith("--target="),
  );
  ...
  return yield* (stray !== undefined
    ...
    : malformed !== undefined
      ? Effect.fail(new UsageFault({ wanted: `--target wants <project>/<config>/<token>, got ${malformed}` }))
      : handler === undefined
        ? Effect.fail(new UsageFault({ wanted: USAGE }))
