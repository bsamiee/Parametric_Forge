...
<div style="padding: 12px 14px; border: 1px solid #1f2937; border-radius: 12px; background: #0b111a;">
  ...
  - **Python:** 3.15 GIL build with uv, ruff, ty, mypy, and `forge-scientific-env`; ruff/ty/mypy resolve project-local versions first through the shim and fall back to machine builds; caches under XDG (`modules/home/environments/languages.nix`). `MACOSX_DEPLOYMENT_TARGET` follows the nix-darwin minimum and is currently `14.0`.
