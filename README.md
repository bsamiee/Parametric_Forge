...
`modules/home/programs/shell-tools/mcp-fleet.nix` is the single fleet manifest: one row declares transport, spawn line or endpoint, env key names, probe class, launcher pin, authentication mode, tool-approval posture, client expectations, and host-OS admission. Every switch on either host runs `forge-mcp reconcile claude` and `forge-mcp reconcile codex` over the host-filtered rows, replacing only manifest-owned MCP maps while preserving unrelated client state and app-private Codex rows. `forge-mcp doctor --network` joins endpoint health to declared credentials, `forge-mcp drift` proves the live projections, `forge-mcp outdated` surfaces stale pins across every engine (npm, PyPI, git HEAD), and the daily `forge-mcp advance` agent rewrites outdated pins, proves the build, and auto-commits — the next switch lands them. Add or extend a server as one manifest row.
