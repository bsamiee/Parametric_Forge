...
local function get_tab_colors(tab, pane, hover)
    ...
    elseif pane.domain_name and host_bg[pane.domain_name] then
        ...
        -- Process-based coloring for inactive tabs
        local process, _ = get_process_info(tab)
        if process then
            local process_colors = {
                ...
                ["python3"] = colors.cyan,
                ["ruby"] = colors.cyan,
                -- Text editors - purple family (creative)
                ["nvim"] = colors.purple,
                ["vim"] = colors.purple,
                ["code"] = colors.purple,
                ["emacs"] = colors.purple,
                -- Version control - pink family (workflow)
                ["git"] = colors.pink,
                ["lazygit"] = colors.pink,
                ["gh"] = colors.pink,
                -- System monitoring - green family (operational)
                ["htop"] = colors.green,
                ["btop"] = colors.green,
                ["ssh"] = colors.green,
            }
            local process_color = process_colors[process]
            if process_color then
