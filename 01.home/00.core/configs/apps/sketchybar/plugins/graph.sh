...
# ----------------------------------------------------------------------------
# Comprehensive system monitoring with popup for SketchyBar
...
collect_system_data() {
    ...
    local probe=$(/bin/ps -Aceo pid,pcpu,comm -r | awk 'NR==2')
...
# Update main graph display
update_main_graph() {
    local graphpoint=$(bc <<<"scale=1; $CPU_PERCENT / 100 ")
    ...
    # Update label
    local graphlabel="${TOP_PERCENT}% - $TOP_PROCESS [$TOP_PID] | $(printf '%.1f' "$POWER_WATTS")W"
    sketchybar --set "$NAME.label" label="$graphlabel" label.color="$label_color"
}

# Update popup items if they exist
update_popup_metrics() {
    local popup_items=("$NAME.cpu" "$NAME.memory" "$NAME.disk" "$NAME.network" "$NAME.power")
    
    # Check if popup exists
    if sketchybar --query "$NAME.cpu" >/dev/null 2>&1; then
        # CPU line
        local cpu_line=$(format_metric_line "CPU" "$CPU_PERCENT" "$TOP_PROCESS" "cpu")
        sketchybar --set "$NAME.cpu" label="$cpu_line"
        
        # Memory line  
        local mem_line=$(format_metric_line "Memory" "$MEMORY_PERCENT" "${MEMORY_USED}M used" "memory")
        sketchybar --set "$NAME.memory" label="$mem_line"
        
        # Disk line
        local disk_line=$(format_metric_line "Disk" "$DISK_PERCENT" "$DISK_AVAIL avail" "disk")
        sketchybar --set "$NAME.disk" label="$disk_line"
        
        # Network line
        local net_line=$(format_metric_line "Network" "$NETWORK_PERCENT" "$(printf '%.1f' "$NETWORK_ACTIVITY")MB/s" "network")
        sketchybar --set "$NAME.network" label="$net_line"
        
        # Power line
        local power_line=$(format_metric_line "Power" "$POWER_PERCENT" "$(printf '%.1f' "$POWER_WATTS")W" "cpu")
