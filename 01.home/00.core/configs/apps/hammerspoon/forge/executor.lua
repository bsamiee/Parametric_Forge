...
local dryRun = true
function M.setDryRun(b) dryRun = b and true or false end
function M.isDryRun() return dryRun end
...
local function spaceIdToIndex(spaceId)
  ...
  for _, s in ipairs(arr) do
    if s.id == spaceId then return s.index end
  end
  return nil
end

local function applyGrid(winId, anchor)
  if not config.gridScript or #config.gridScript == 0 then return end
...
local function scheduleWindow(winId, ms, fn)
  ...
  if bucket and bucket.timer then bucket.timer:stop() end
  bucket = bucket or { ops = {} }
  bucket.timer = hs.timer.doAfter(ms/1000, function()
    local ok, err = pcall(fn)
    if not ok then log.w("window flush error: " .. tostring(err)) end
  end)
...
local function scheduleSpace(spaceId, ms, fn)
  ...
  bucket.timer = hs.timer.doAfter(ms/1000, function()
    local ok, err = pcall(fn)
    if not ok then log.w("space flush error: " .. tostring(err)) end
  end)
...
function M.windowSticky(winId, sticky)
  if not M.sa.available then return end
  scheduleWindow(winId, config.debounce.window, function()
    if sticky then
      local info = getWindowInfo(winId)
...
function M.windowSubLayer(winId, layer)
  if not M.sa.available then return end
  if layer ~= "above" and layer ~= "below" and layer ~= "normal" then return end
  scheduleWindow(winId, config.debounce.window, function()
...
function M.normalizeSpace(spaceId)
  scheduleSpace(spaceId, config.debounce.space, function()
    ...
    if idx then
      ...
      if config.space.autoBalance then yabai(string.format("space %d --balance", idx)) end
    end
