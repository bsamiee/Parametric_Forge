...
local M = {}
local log = hs.logger.new("forge.policy", hs.logger.info)

local function axRole(win)
  local ok, ax = pcall(hs.axuielement.windowElement, win)
  if not ok or not ax then return nil, nil end
...
function M.applyWindowPolicy(win)
  ...
  if rule then
    if rule.manage == false then exec.windowFloat(id, true) end
    if rule.sticky == true then exec.windowSticky(id, true) end
    if rule.subLayer then exec.windowSubLayer(id, rule.subLayer) end
    if rule.gridAnchor then exec.windowGrid(id, rule.gridAnchor) end
    return
  end

  -- Generic float policy for dialogs/sheets/system dialogs & tiny windows
  local isDialog = (role == "AXWindow" and (
    subrole == "AXDialog" or subrole == "AXSheet" or subrole == "AXSystemDialog" or subrole == "AXPopover" or subrole == "AXInspector" or subrole == "AXUnknown"))

  -- Picture-in-Picture or system floating windows across apps
  local isSystemFloating = (subrole == "AXSystemFloatingWindow")

  -- Preferences/Settings generic titles
  local isPrefsTitle = false
  if title and #title > 0 then
    isPrefsTitle = (string.match(title, "Preferences")
      ...
      or string.match(title, "^(General|(Tab|Password|Website|Extension)s|AutoFill|Se(arch|curity)|Privacy|Advance)$")) and true or false
  end
...
function M.applySpacePolicy(spaceId)
  ...
  local sh = function(cmd)
    return hs.execute("/usr/bin/env PATH='/opt/homebrew/bin:/usr/local/bin:/run/current-system/sw/bin:" .. os.getenv("PATH") .. "' sh -lc '" .. cmd .. "'", true)
  end
