...
rec {
  ...
  notify =
    ...
    let
      ...
      icon =
        if appIcon != null then appIcon else "/System/Applications/Utilities/Terminal.app/Contents/Resources/Terminal.icns";
      ...
      optionalArgs = lib.filter (x: x != "") [
        ...
        (lib.optionalString (actions != null)
          "-actions ${lib.concatStringsSep "," (map (a: "'${lib.escape [ "'" ] a}'") actions)}"
        )
