...
const SECRETS_LAW =
    'SECRETS DISCIPLINE (absolute): configs you read contain live token values — NEVER reproduce any secret VALUE in the dossier or ' +
...
const LANES = [
    {
        ...
        task:
            ...
            'LOCAL READS (cite file:line): ' +
            FORGE +
            '/modules/home/programs/shell-tools/1password.nix IN FULL (the env.template op:// key inventory = the migration keyset, guiOpSecrets launchd agent, injectSecretsFromVault activation), ' +
            FORGE +
            '/.claude/hooks/setup-env.sh (canonical agent hook), and run doppler --version && doppler projects --json (safe: names only). ' +
    ...
    },
    {
        ...
        task:
            ...
            'LOCAL READS: /Users/bardiasamiee/.claude.json mcpServers block (server names + command/args; <redacted> every env value); /Users/bardiasamiee/.claude/settings.json (permission grants incl. suspected-dead mcp__computer-use__* and mcp__postgres__*, and extraKnownMarketplaces rasm-lsp hardcoded to ' +
            RASM +
            '/.claude/lsp-marketplace); ' +
            '/Users/bardiasamiee/.codex/config.toml mcp_servers tables (names + commands, redact env values); the Forge npm-MCP launcher owner from commit 13e654d (rg "forge-mcp-outdated|mcp-launchers|forge-tavily-mcp" ' +
            FORGE +
            '/modules -l, read the owning file fully — table-driven pinned launchers); ' +
            'and Rasm recent additions: rg -i "playwright|e2e|browser" ' +
            RASM +
            '/.claude ' +
            RASM +
            '/CLAUDE.md ' +
            RASM +
            '/AGENTS.md plus any mcp config files found — inventory exactly which playwright/e2e MCP servers or skills Rasm added, their launch commands and pins. ' +
    ...
    },
    {
        ...
        task:
            ...
            'INVENTORY (the product is a matrix): compare ' +
            FORGE +
            '/.claude, ' +
            RASM +
            '/.claude, ' +
            MAGHZ +
            '/.claude, /Users/bardiasamiee/.claude — subtrees skills/, agents/, commands/, hooks/, scripts/, output-styles/, workflows/, settings.json, settings.local.json. ' +
            ...
            'List one-tree-only files per tree. Read ' +
            FORGE +
            '/.claude/settings.local.json fully (suspected Parametric_Portal leftovers — verify against ' +
            GH +
            '/Parametric_Portal if present). ' +
            'Identify the shell scripts under ' +
            FORGE +
            '/.claude (hooks/*.sh, scripts/*.sh) — the user calls them "the 2 scripts" — read each fully and grade (strict mode, shellcheck-cleanliness, correctness). ' +
    ...
    },
    {
        ...
        task:
            ...
            'LOCAL READS: .claude/hooks/setup-env.sh in ' +
            FORGE +
            ', ' +
            RASM +
            ', ' +
            MAGHZ +
            ', and locate the other two repos with fd -t d -d 1 . ' +
            GH +
            ' (Noesis and Parametric_Portal); read their variants too. ' +
            ...
            'Extract the EXACT corrupted associative-array lines in ' +
            RASM +
            "/.claude/scripts/bootstrap-cli-tools.sh (keys like [trash - put], [antigravity - installer], [github - go], [github - release - sha]) and the corresponding intact lines in Forge's copy (locate it under " +
            FORGE +
            '/.claude or /Users/bardiasamiee/.claude — rg -l bootstrap-cli-tools across all trees). ' +
...
const wrapperPrompt = (lane) => {
    ...
    return (
        ...
        '(2) Write tool: create ' +
        taskFile +
        ' containing EXACTLY the task text between the markers below (the markers themselves are NOT part of the file): ' +
        '<<<TASK ' +
        lane.task +
        ' TASK>>> Then create ' +
        schemaFile +
        ...
        stderrFile +
        ' & ' +
        '(4) Poll with sequential bounded Bash calls (each its own tool call: sleep 45; test -s ' +
        reportFile +
        ' && echo READY || (pgrep -f "' +
        base +
        '-report" >/dev/null && echo ALIVE || echo GONE)). An absent report while the process lives is NORMAL — keep polling patiently; you have a raised stall allowance for exactly this. Hard deadline ' +
        DEADLINE_MIN +
        ...
        reportFile +
        ' >/dev/null (invalid json = failure with stderr tail); then jq -r .dossier_markdown ' +
        reportFile +
        ' > ' +
        dossierMd +
        ' && cp ' +
