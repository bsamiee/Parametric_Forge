export const meta = {
    name: 'dossier-wave',
    description:
        'Thread 1+2 dossier fan-out: gpt-5.5 lanes ground the terminal keystone and nix-core rebuild with verified current-state and upstream research',
    whenToUse: 'Before executing a rebuild thread: builds the durable dossiers the fable executor consumes',
    phases: [{ title: 'Dossier', model: 'sonnet' }],
};
...
const LANES = [
    {
        scope: 't1-wezterm',
        task:
            'GOAL: build the WezTerm-nightly migration dossier for a tabula-rasa terminal rebuild on macOS (Apple Silicon, nix-darwin + Home Manager). ' +
            'LOCAL READS (read fully, cite file:line): ' +
            FORGE +
            '/modules/home/programs/apps/wezterm/ (all .lua + default.nix), ' +
            FORGE +
            '/modules/darwin/homebrew/casks.nix (wezterm cask row), ' +
            FORGE +
            '/modules/darwin/homebrew/default.nix (autoupdate posture). ' +
    ...
    },
    {
        scope: 't1-zellij',
        task:
            'GOAL: build the Zellij tabula-rasa grounding dossier. Zellij is 0.44.3, config is Nix-generated. ' +
            'LOCAL READS (read fully, cite file:line): ' +
            FORGE +
            '/modules/home/programs/apps/zellij/ (config.nix, default.nix, layouts/*.nix, themes/*.nix), ' +
            FORGE +
            '/modules/home/programs/apps/wezterm/integration.lua (handoff), ' +
            FORGE +
            '/modules/home/environments/applications.nix (ZELLIJ_DEFAULT_LAYOUT), ' +
    ...
    },
    {
        scope: 't1-yazi',
        task:
            'GOAL: build the Yazi modernization dossier (floating-popup-first redesign, current plugin ecosystem). ' +
            'LOCAL READS (read fully, cite file:line): ' +
            FORGE +
            '/modules/home/programs/apps/yazi/ (every file incl. yazi.toml, theme, plugins/*), ' +
            FORGE +
            '/modules/home/scripts/terminal.nix  and ' +
            FORGE +
            '/modules/home/scripts/terminal.nix  (forge-yazi.sh, forge-edit.sh, forge-nvim.sh sources — extract the full text of each with line numbers). ' +
    ...
    },
    {
        scope: 't1-karabiner',
        task:
            'GOAL: build the Karabiner capture + declarative-staging dossier. The live config is the ONLY source of the physical modifier scheme every zellij chord depends on. ' +
    ...
    },
    {
        scope: 't1-whichkey',
        task:
            'GOAL: build the keybinding-discoverability stack dossier for Zellij 0.44.3 (decision locked: in-terminal stack = compact-bar tooltip + zjstatus-hints + zellij-forgot, dracula-themed). ' +
            'LOCAL READS: ' +
            FORGE +
            '/modules/home/programs/apps/zellij/config.nix (plugin alias block + zjstatus config, keybind modes) for wiring shape. ' +
    ...
    },
    {
        scope: 't1-editrail',
        task:
            'GOAL: design-ground the yazi->editor rail rebuild (current one is broken three ways: focus-next-pane hits yazi at startup, Nix-escaped \\$SOCKET/\\$* expand empty, ordinal focus is layout-dependent). ' +
            'LOCAL READS (extract full text with line numbers): ' +
            FORGE +
            '/modules/home/scripts/terminal.nix (forge-edit.sh, forge-nvim.sh), ' +
            FORGE +
            '/modules/home/scripts/terminal.nix  (forge-yazi.sh, any zoxide script), ' +
            FORGE +
            '/modules/home/programs/apps/yazi/yazi.toml (opener config), ' +
            'and check whether an nvim module exists under ' +
            FORGE +
            '/modules/home/programs/apps/nvim/ (read its default.nix if so). ' +
    ...
    },
    {
        ...
        task:
            'GOAL: build the Determinate-Nix declarative-settings migration dossier. Today nix.enable=false makes the entire nix.settings block dead; /etc/nix/*.conf are stale installer artifacts. Decision locked: adopt the official Determinate darwin module. ' +
            'LOCAL READS (read fully, cite line numbers): ' +
            FORGE +
            '/modules/common/nix.nix (the dead block — every setting), ' +
            FORGE +
            '/flake.nix + ' +
            FORGE +
            '/flake-modules/nixpkgs.nix (input wiring), /etc/nix/nix.conf and /etc/nix/nix.custom.conf (live state), and run: nix --version; determinate-nixd version 2>/dev/null. ' +
    ...
    },
    {
        scope: 't2-cache',
        task:
            'GOAL: build the build-speed dossier: kill the 1h+ rebuilds. Confirmed causes: uv overlay overrideAttrs (version/src/cargoHash swap -> full local Rust build), a mise checkFlags override (cache-bust), cachix push doubly dead. Decision locked: drop absorbed overrides, push explicitly from forge-redeploy, and check community caches/overlays that ship prebuilt binaries. ' +
            'LOCAL READS (cite line numbers): ' +
            FORGE +
            '/overlays/default.nix (uv override block), the mise override (search ' +
            FORGE +
            '/modules for mise.nix overrideAttrs), ' +
            FORGE +
            '/flake.lock (nixpkgs rev + date), ' +
            FORGE +
            '/modules/home/programs/shell-tools/forge-tools.nix (forge-redeploy body). ' +
    ...
    },
    {
        scope: 't2-nh',
        task:
            'GOAL: build the rebuild-driver dossier: forge-redeploy wrapping nh v4 with dix/nvd diffs on Determinate Nix. ' +
            'LOCAL READS (cite line numbers): ' +
            FORGE +
            '/modules/home/programs/shell-tools/forge-tools.nix (current forge-redeploy: pkgs.nix pin in runtimeInputs causes eval-cores/lazy-trees unknown-setting warnings, sudo -n darwin-rebuild switch path), ' +
            FORGE +
            '/modules/darwin/settings/security.nix (TouchID/sudo posture). ' +
...
const PRODUCT_SCHEMA = {
    ...
    properties: {
        dossier_markdown: {
            type: 'string',
            description:
                'The complete dossier as standalone markdown: current-state extractions with file:line cites, research findings with source URLs, config-ready snippets, tables. This is the primary product.',
        },
        facts: {
            type: 'array',
            items: {
...
const wrapperPrompt = (lane) => {
    ...
    return (
        ...
        '(2) Write tool: create ' +
        taskFile +
        ' containing EXACTLY this task text (verbatim, nothing added): <<<TASK ' +
        lane.task +
        ' TASK>>> then create ' +
        schemaFile +
        ...
        schemaFile +
        ' . ' +
        '(3) Launch detached (ONE Bash call, exactly this shape): cd ' +
        FORGE +
        ...
        stderrFile +
        ' & ' +
        '(4) Poll with sequential bounded Bash calls (each its own tool call, e.g. sleep 45; test -s ' +
        reportFile +
        ' && echo READY || (pgrep -f "' +
        base +
        '-report" >/dev/null && echo ALIVE || echo GONE)). An absent report while the process lives is NORMAL — keep polling. Hard deadline ' +
        DEADLINE_MIN +
        ' minutes: alive past it with no report = WEDGED — pkill -f "' +
        base +
        '-report", relaunch once (repeat step 3); a second wedge or a GONE with empty report after one relaunch = failure: return ok=false with failure = last 3 lines of ' +
        stderrFile +
        ' . ' +
        '(5) On READY: Bash: jq -e .dossier_markdown ' +
        reportFile +
        ' >/dev/null (invalid json = treat as failure with stderr tail); then jq -r .dossier_markdown ' +
        reportFile +
        ' > ' +
        dossierMd +
        ' && cp ' +
        ...
        dossierJson +
        ' . ' +
        '(6) Compute MECHANICALLY: entries = jq "(.facts|length) + (.versions|length)" ' +
        reportFile +
        ' ; headline = "facts=" + (.facts|length) + " versions=" + (.versions|length) + " risks=" + (.risks|length) + " oq=" + (.open_questions|length) via jq string interpolation. ' +
        '(7) Return the receipt object: ok=true, report=' +
        dossierMd +
        ', entries, headline, failure="".'
    );
};
...
const roster = (
    await parallel(
        LANES.map(
            (lane) => () =>
                agent(wrapperPrompt(lane), {
                    label: 'gpt-5.5:' + lane.scope,
                    phase: 'Dossier',
                    schema: RECEIPT,
                    model: 'sonnet',
                    effort: 'low',
                    ...
                    lane: lane.scope,
                    scope: [lane.scope],
                    ...(r || { ok: false, report: '', entries: 0, headline: '', failure: 'wrapper skipped or died' }),
                })),
        ),
    )
).filter(Boolean);
