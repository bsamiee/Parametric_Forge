export const meta = {
    name: 'thread-review',
    description:
        'Sequential cold write-review station: a predicate-positive CRITIQUE pass then a predicate-negative RED-TEAM pass, both fable write-agents that fix and extend in place and leave fix-log dossiers',
...
const COMMON =
    'Repo: ' +
    FORGE +
    ' (nix-darwin + Home Manager flake, macOS Apple Silicon). Thread ' +
    THREAD +
    ' territory (the ONLY paths you may edit): ' +
    TERRITORY.join(', ') +
    ' — plus the fix-log you write under ' +
    LOGDIR +
    '. Scope of record: git diff ' +
    BASE +
    '..' +
    HEAD +
    ' restricted to that territory, ' +
    'PLUS any uncommitted working-tree changes inside it. Ground truth inputs, read IN FULL before editing: the thread dossiers (' +
    DOSSIERS.join(', ') +
    ') ' +
...
const critique = await agent(
    COMMON +
        CRITIQUE_MANDATE +
        'Write your fix-log to ' +
        LOGDIR +
        '/' +
        THREAD +
        '-critique.md (mkdir -p first). ' +
...
const ammoNote = AMMO.length
    ? 'READER AMMO (independent read-only findings from other model lineages — treat every claim as unverified until disk proves it; refute freely): ' +
      AMMO.join(', ') +
      '. '
    : '';
const redteam = await agent(
    COMMON +
        REDTEAM_MANDATE +
        ammoNote +
        'The critique pass before you wrote ' +
        LOGDIR +
        '/' +
        THREAD +
        '-critique.md — consult it ONLY AFTER your own cold attack; re-verify its claims like any other. ' +
        'Write your fix-log to ' +
        LOGDIR +
        '/' +
        THREAD +
        '-redteam.md. Return the receipt: log path, counts, gates_green, summary.',
