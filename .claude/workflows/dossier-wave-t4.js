export const meta = {
    ...
    description:
        'Thread 4 dossier fan-out with launch-only wrappers and an orchestrator harvest loop: darwin fixes, machine cleanup gates, Maghz VPS wiring, fonts/VSCode integration',
...
const LANES = [
    {
        scope: 't4-darwin',
        task:
            'GOAL: build the darwin-system fix dossier. Locked decisions: login window flips to icon list (SHOWFULLNAME=false); wallpaper becomes a NET-NEW home-manager user-scope activation (nothing exists today); security posture stays fully-off (documented consent — do not propose tightening); dock pins get real install sources or removal. ' +
            'LOCAL READS (cite file:line): every file under ' +
            FORGE +
            '/modules/darwin/ (system.nix, interface.nix, security.nix, fonts.nix, input.nix, homebrew/*, default.nix, settings/*). Also read /run/current-system/activate excerpts if useful (rg for loginwindow, LSQuarantine, AppleFontSmoothing). ' +
    ...
    },
    {
        scope: 't4-cleanup',
        task:
            'GOAL: build the machine-cleanup gate dossier. Authorized cleanups: chmod 700 ~/Library/LaunchAgents; delete ~/.pyenv; purge zsh compdump litter + add a guard; retire rustup (~/.cargo + ~/.rustup, 3.1GB) — the rustup retirement is GATED on proving no sibling repo needs it. ' +
            'LOCAL SWEEPS (read-only, cite evidence): (1) rustup gate: fd rust-toolchain.toml across ' +
            GH +
            ' (all repos); rg -l "cargo |rustc|cargo build|cargo run" in each sibling repo\'s build scripts/justfiles/CI configs/docs; check ~/.cargo/bin contents and which -a rustc cargo; determine what used cargo recently (mtimes under ~/.cargo/registry). Verdict: safe-to-retire or name the blocker. ' +
    ...
    },
    {
        scope: 't4-maghz',
        task:
            'GOAL: build the Maghz VPS-primary wiring dossier. Locked decisions: the Hostinger VPS is the durable home for the Postgres/Ollama/n8n stack; the Mac gets LocalForwards 15435/11434/5678; a local-parity mode must exist (same compose stack locally, explicit profile switch, no silent DSN drift); the VPS consumes secrets via a Doppler service token; the Codex postgres MCP must stop silently failing. ' +
            'LOCAL READS (cite file:line): ' +
            FORGE +
            '/modules/home/programs/shell-tools/ssh.nix (current LocalForwards 9000/6800/1455 + IdentityAgent wiring); ' +
            MAGHZ +
            ' repo — read the deploy surface (compose files, admin/remote.py or equivalent, settings.py DSN handling around lines 94 and 235, infra.py port bindings), plus /Users/bardiasamiee/.codex/config.toml postgres MCP entry (redact env values — key names only). ' +
    ...
    },
    {
        scope: 't4-fonts',
        task:
            'GOAL: build the fonts + GUI-app integration dossier (user pain: VSCode and other GUI apps do not see the Nix-installed families like GeistMono Nerd Font; user wants fonts genuinely usable everywhere; VSCode settings.json is 12.5KB and fully unmanaged). ' +
            'LOCAL READS (cite file:line): ' +
            FORGE +
            '/modules/darwin/fonts.nix; ls "/Library/Fonts/Nix Fonts" (top level only); ~/Library/Application Support/Code/User/settings.json (extract ONLY font/theme-related keys + total key count — do not dump the whole file); check VSCode version (code --version if available). ' +
...
const PRODUCT_SCHEMA = {
    ...
    properties: {
        dossier_markdown: {
            type: 'string',
            description:
                'The complete dossier as standalone markdown: current-state extractions with file:line cites, research findings with source URLs, config-ready snippets, tables.',
        },
        facts: {
            type: 'array',
            items: {
...
const launchPrompt = (lane) => {
    ...
    return (
        ...
        '(2) Write tool: create ' +
        taskFile +
        ' containing EXACTLY the task text between the markers (markers are NOT part of the file): <<<TASK ' +
        lane.task +
        ' TASK>>> ' +
        'Then create ' +
        schemaFile +
        ...
        stderrFile +
        ' & ' +
        '(4) ONE verification Bash call: pgrep -f "' +
        base +
        '-report" >/dev/null && echo ALIVE || echo DEAD-ON-LAUNCH. ' +
        '(5) Return the receipt NOW: ok=true (if ALIVE), report=' +
        reportFile +
        ', entries=0, headline="launched", failure="" — or ok=false with the stderr tail if DEAD-ON-LAUNCH.'
    );
};

const harvestPrompt = (pending) =>
    SAFETY +
    'You are a mechanical harvester for detached codex dossier lanes. Lanes pending: ' +
    JSON.stringify(pending) +
    ' . For EACH lane L do exactly: ' +
    '(a) if ' +
    DOSSIERS +
    '/L.md already exists (test -s), count it promoted; ' +
    '(b) else if ' +
    SCRATCH +
    '/L-report.json exists AND jq -e ".dossier_markdown | length > 100" passes: promote via jq -r .dossier_markdown ' +
    SCRATCH +
    '/L-report.json > ' +
    DOSSIERS +
    '/L.md && cp ' +
    ...
    '/L-report.json ' +
    DOSSIERS +
    '/L.json — count promoted; ' +
    '(c) else if pgrep -f "L-report" finds the process: count pending; ' +
    '(d) else re-check (b) once more (write race), and if still no valid report count dead as "L: <last 2 lines of ' +
    SCRATCH +
    '/L-stderr.log>". ' +
...
const launches = (
    await parallel(
        LANES.map(
            (lane) => () =>
                agent(launchPrompt(lane), {
                    label: 'gpt-5.5:' + lane.scope,
                    phase: 'Launch',
                    schema: LAUNCH_RECEIPT,
                    model: 'sonnet',
                    effort: 'low',
