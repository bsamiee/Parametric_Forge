...
- Bump past high surgically, one axis at a time: a hard single-context leg takes xhigh or max. The subagent cap is `agents.max_threads` while `multi_agent_v2` is disabled; the active v2 cap is `features.multi_agent_v2.max_concurrent_threads_per_session`.
...
A row-shaped sweep — one similar task per work item — rides ONE lane's `spawn_agents_on_csv` tool instead of N processes: the lane builds a CSV, spawns one worker subagent per row under the active agent-concurrency cap, and exports a combined results CSV (`output_schema` types each worker's row; `codex exec` streams one-line batch progress on stderr). Under `-s read-only` the workers return results in-band but the `output_csv_path` export never lands — a lane that needs the CSV artifact runs `workspace-write`. Reusable worker personas land as custom agent TOML files under `~/.codex/agents/` — `name`, `description`, `developer_instructions`, with optional `model`, `model_reasoning_effort`, and `sandbox_mode` overrides — and the prompt spawns them by name.
...
`agent()` accepts only Claude models — a workflow dispatches codex through a thin wrapper agent (`model: 'sonnet', effort: 'low'`, label prefixed with the real worker: `terra:`, `sol:`, or `luna:`) whose whole job is one `codex` MCP tool call and a receipt: load the tool (`ToolSearch` `select:mcp__codex__codex`), call it ONCE with the self-contained task as `prompt`, model and sandbox and `cwd` pinned, effort via `config`; write the returned product VERBATIM to the lane's scratch report path with the Write tool; return the thin receipt {ok, report, entries, headline, failure} — never relaying, re-doing, or re-judging the work. The blocking tool call IS the wait — no launch receipts, no polling, no harvest rounds — and the wrapper's `schema` option is the validation boundary, so the codex prompt carries a prose JSON contract and no schema file exists on this path. What a wrapper cannot do is idle BETWEEN calls: an agent turn that emits no-op calls while waiting on out-of-band state trips no-progress enforcement; a live blocking call never does.

Wrapper economics rule the lane count: each wrapper agent costs a full context spin-up (~75k tokens measured) regardless of effort. Short legs batch — one wrapper makes several sequential `codex` calls and returns one combined receipt; a row-shaped batch collapses into a single lane whose codex session runs `spawn_agents_on_csv` ([10]); an iterative chain continues one thread through `codex-reply` with the returned `threadId` instead of re-paying exploration. A lane expected to outrun the MCP tool timeout is the one case that still runs the detached CLI form ([07]) — from the MAIN loop as a `run_in_background` Bash keeper, never inside a workflow wrapper. Codex tokens never count toward `budget.spent()`.
