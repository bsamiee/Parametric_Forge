...
Prompt and agent handlers bind only to the judgment-eligible events: `PermissionDenied`, `PermissionRequest`, `PostToolBatch`, `PostToolUse`, `PostToolUseFailure`, `PreToolUse`, `Stop`, `SubagentStop`, `TaskCompleted`, `TaskCreated`, `TeammateIdle`, `UserPromptExpansion`, `UserPromptSubmit`. Every other event takes `command`, `http`, and `mcp_tool`, except `SessionStart` and `Setup`, which take `command` and `mcp_tool` only.
