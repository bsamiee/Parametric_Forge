...
function runTask(launchPath, args) {
  ...
  return { status: task.terminationStatus, stdout: decode(out.fileHandleForReading), stderr: decode(err.fileHandleForReading) }
}
...
function writeUTF8(text, posixPath) {
  ...
  if (!ok) { throw new Error(error.localizedDescription.js) }
  return target.js
}
...
function perform(bundleID, payload) {
  ...
  const rows = documents().map((document) => ({ name: document.name(), path: document.path() }))
...
function run(argv) {
  try {
    ...
    if (!bundleID) { throw new Error('missing bundle identifier') }
...
function openDocuments(dropped) {
  return JSON.stringify({ ok: true, dropped: dropped.map((item) => item.toString()) })
}
