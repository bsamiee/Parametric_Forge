...
const VERDICT = { 0: 'granted', '-1743': 'denied', '-1744': 'undecided', '-600': 'target-not-running' }
...
function run(argv) {
  ...
  if (!bundleID) { return JSON.stringify({ ok: false, error: 'usage: <bundle-id> [class-hex] [id-hex]' }) }
  const scope = { eventClass: klass ? parseInt(klass, 16) : WILDCARD, eventID: ident ? parseInt(ident, 16) : WILDCARD }
  return JSON.stringify({ ok: true, ...consentFor(bundleID, scope.eventClass, scope.eventID) })
}
